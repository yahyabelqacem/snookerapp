...
export default function TablePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [fouls1, setFouls1] = useState<number[]>([]);
  const [fouls2, setFouls2] = useState<number[]>([]);

  const breaksRef = useRef([0, 0]);
  const bestsRef = useRef([0, 0]);
  const balls1Ref = useRef<BallEntry[]>([]);
  const balls2Ref = useRef<BallEntry[]>([]);
  const fouls1Ref = useRef<number[]>([]);
  const fouls2Ref = useRef<number[]>([]);
    ...
    supabase.from("game_state").select("*").eq("id", tableId).single()
      .then(({ data }) => {
        if (data) {
          ...
          scoresRef.current = [data.score1, data.score2];
          balls1Ref.current = Array.isArray(data.balls1) ? data.balls1 : [];
          balls2Ref.current = Array.isArray(data.balls2) ? data.balls2 : [];
          fouls1Ref.current = Array.isArray(data.fouls1) ? data.fouls1 : [];
          fouls2Ref.current = Array.isArray(data.fouls2) ? data.fouls2 : [];
          setFouls1(fouls1Ref.current);
          setFouls2(fouls2Ref.current);
  ...
  const startGame = async () => {
    ...
    fouls1Ref.current = []; fouls2Ref.current = [];
  ...
  const syncToSupabase = async (
    ...
    await supabase.from("game_state").update({
      ...
      fouls1: fouls1Ref.current, fouls2: fouls2Ref.current,
      updated_at: new Date().toISOString()
    }).eq("id", tableId);
  };
  ...
  const addScore = (pts: number) => {
    ...
    const newScores = scoresRef.current.map((s, i) => i === p ? s + pts : s);
    scoresRef.current = newScores;
    setHistory(h => [...h, { player: p, pts, breakBefore: newBreaks[p] - pts, bestBefore: bestsRef.current[p], isEndBreak: false }]);
  ...
  const addFoul = (pts: number) => {
    ...
    if (active === 0) {
      fouls1Ref.current = [...fouls1Ref.current, pts];
      setFouls1([...fouls1Ref.current]);
    } else {
      fouls2Ref.current = [...fouls2Ref.current, pts];
      setFouls2([...fouls2Ref.current]);
    }
    const newBreaks = [...breaksRef.current]; newBreaks[other] += pts; breaksRef.current = newBreaks;
    const newScores = scoresRef.current.map((s, i) => i === other ? s + pts : s);
    scoresRef.current = newScores;
    setHistory(h => [...h, { player: other, pts, breakBefore: newBreaks[other] - pts, bestBefore: bestsRef.current[other], isEndBreak: false, isFoul: true }]);
    setScores(newScores); setBreaks([...newBreaks]);
    syncToSupabase(newScores, newBreaks, bestsRef.current, active, name1, name2);
  };
  ...
  const switchPlayer = (to: number) => {
    ...
    syncToSupabase(scoresRef.current, newBreaks, newBests, to, name1, name2);
  };
  ...
  const undo = () => {
    ...
    if (last.isEndBreak) {
      ...
      syncToSupabase(scoresRef.current, newBreaks, newBests, last.player, name1, name2);
    } else if (last.isFoul) {
      const fouler = active === 0 ? 1 : 0;
      if (fouler === 0) { fouls1Ref.current = fouls1Ref.current.slice(0, -1); setFouls1([...fouls1Ref.current]); }
      else { fouls2Ref.current = fouls2Ref.current.slice(0, -1); setFouls2([...fouls2Ref.current]); }
      const newScores = scoresRef.current.map((s, i) => i === last.player ? s - last.pts : s);
      scoresRef.current = newScores;
      const newBreaks = [...breaksRef.current]; newBreaks[last.player] = last.breakBefore; breaksRef.current = newBreaks;
      setScores(newScores); setBreaks([...newBreaks]);
      syncToSupabase(newScores, newBreaks, bestsRef.current, active, name1, name2);
  ...
  const confirmFinDeFrame = async () => {
    ...
    await supabase.from("frames").insert({
      ...
      winner_score: s[winnerIdx], loser_score: s[loserIdx],
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 28 }}>
            <div style={{
              fontSize: "clamp(28px, 5vw, 42px)", fontFamily: "'Times New Roman', serif",
          ...
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", letterSpacing: 3, textTransform: "uppercase", marginBottom: 24 }}>Enter Players</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 28 }}>
            <div style={{ textAlign: "left" }}>
              ...
              <input value={tempName1} onChange={e => setTempName1(e.target.value)}
            ...
            <div style={{ textAlign: "left" }}>
              ...
              <input value={tempName2} onChange={e => setTempName2(e.target.value)}
                onKeyDown={e => e.key === "Enter" && canStart && startGame()}
          ...
          <button onClick={startGame} disabled={!canStart}
            style={{ width: "100%", padding: "16px", borderRadius: 14, border: "none",
              ...
              color: canStart ? "#fff" : "rgba(255,255,255,0.2)", fontSize: 15, fontWeight: 600,
              cursor: canStart ? "pointer" : "default", letterSpacing: 3, textTransform: "uppercase",
  ...
  return (
    <div style={{ background: "#0d0d0f", minHeight: "100vh", padding: "28px 24px", fontFamily: "sans-serif" }}>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "#17171f", borderRadius: 16, padding: 32, maxWidth: 340, width: "90%", border: "1px solid #2a2a36", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 12, color: "#444", marginBottom: 20 }}>
              <span style={{ color: "#E24B4A" }}>{names[scores[0] > scores[1] ? 1 : 0]}</span>{" "}ytbeddel b{" "}
          ...
          <div style={{ background: "#17171f", borderRadius: 16, padding: 24, maxWidth: 380, width: "90%", border: "1px solid #2a2a36" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              ...
              <button onClick={joinQueue} disabled={!queueName.trim()}
                style={{ padding: "10px 16px", borderRadius: 10, border: "none", background: queueName.trim() ? "#1D9E75" : "#2a2a2a", color: "#fff", fontSize: 13, cursor: "pointer", fontWeight: 500 }}>Join</button>
            </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto 8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        ...
        <button onClick={() => { if (playColors) playColorsReset(); }}
          style={{ padding: 13, borderRadius: 10, fontSize: 13, fontWeight: 500, cursor: playColors ? "pointer" : "default", border: `1px solid ${playColors ? "#1a3a1a" : "#3a1a1a"}`, background: playColors ? "#0a1a0a" : "#1a0808", color: playColors ? "#1D9E75" : "#E24B4A" }}>
